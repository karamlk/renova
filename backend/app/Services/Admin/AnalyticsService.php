...
class AnalyticsService
{
    public function getAnalyticsSnapshot(): array
    {
        return Cache::remember(
            ...
            now()->addMinutes(2),
            fn() => $this->buildAnalyticsSnapshot()
        );
    }

    private function buildAnalyticsSnapshot(): array
    {
        ...
        $typeDistribution = DB::table('projects')
            ->join(
                'construction_forms',
                'projects.construction_form_id',
                '=',
                'construction_forms.id'
            )
            ->join(
                'reconstruction_requests',
                'construction_forms.reconstruction_request_id',
                '=',
                'reconstruction_requests.id'
            )
            ->select(
                'reconstruction_requests.type',
        ...
        if ($totalProjects > 0) {
            foreach ($typeDistribution as $item) {
                if (array_key_exists($item->type, $formattedPercentages)) {
                    $formattedPercentages[$item->type] =
                        round(($item->total / $totalProjects) * 100, 2);
                }
            }
        }
